...
The mouse that survived
--------------------------------------------------------------------------------

The point of the following example is to demonstrate the importance of considering the technical context of an experiment when confronted with an extreme observation.

As an example, the Pons lab studies glioblastoma, a brain tumor. They are investigating whether adamantium inhibitors would prolong life in patients with glioblastoma. The group plans a pre-clinical study using a mouse model that expresses an inducible oncogenic form of the *ras* gene in brain tissue. Expression of this ongogene causes the development of glioblastoma with characteristics in common with the human disease. This model is used widely, with many groups showing that after the gene is activated the mice uniformly die from brain tumors after approximately 21 days. 
...
The P value for a log-rank test comparing the life-span post oncogene activation of the control mice, in black, to the drug-treated mice, in blue, is `r round(p.val, 3)`, failing to support the hypothesis that the drug extends life. However, note a data point that appears to defy our expectations: **one of the control animals was still alive at 40 days**, and an necropsy at that time showed it never developed a brain tumor. Given the 100% penetrance of the phenotype, the most likely biological explanation for a mouse surviving this experiment was failure to activate the oncogene. If the oncogene did not activate, then data from this mouse would be a technical error. Re-doing the analysis without the mouse that failed to develop a tumor produces the following result:
...
The following example demonstrates how a single observation can radically alter the interpretation of an larger experiment. The point of the example is to 
...
In this example experiment, two real-valued variables were measured in 11 subjects labeled with the letters "a" through "k". The question of interest was was whether the paired variables were correlated with each other. In this example, we will assume that the experiments were performed correctly and there was no reason to think that any of the measurements was technically flawed.
...
Unlike the previous example, in this case there is no clear technical argument for dropping the extreme point from the analysis. This pathological case shows that a single outlier point can radically change the Pearson correlation, despite not representing the distribution as a whole. 

How can the addition point *k* completely reverse the correlation coefficient from `r cor.10` to `r cor.11`? Recall the definition of correlation:
...
`r signif( sum( (aa-mean(aa)) - (bb-mean(bb)) )/ (11-1), 2)`, moving from negative to positive. This accounts for the change in sign of the correlation coeffcient, from negative (inverse) to positive (direct) correlation.
...
A first step to detecting potential outliers is to look at summaries of the data, and the actual data themselves when this is feasable. This point is so fundamental it bears repeated in larger, bold letters:
...
What should be done about this observation? In the absence of any strong prior information that the data are *fundamentally mistaken* due to a technical error, it would be inappropriate to remove the errant point, partiularly in a *post-hoc* analysis. However, the point is clearly problematic due to its excessive influence. One approach would be to **replace the Pearson correlation with an analysis that is robust to outliers**.
...
One disadvantage of converting to ranks is the **potential for a loss of statistical power** in cases where the assumptions of the pearson correlation are all met. However, this loss of power is often modest, and the potential benefit of reducing erroneous conclusions from extreme values that appear by chance is large.

> To perform a Spearman rank correlation in R measuring correlation between vectors X and Y, use cor.test(X, Y, method="spearman")
...
If the data are not well0behaved, using Pearson correlation can result in a dramatic loss of power. On the left below I show 100 points with a positive correlation. On the right I modify the plot by changing a single point to a much higher value. Note the dramatic effect on the Pearson correlation and the negligible effect on the Spearman rank correlation:
...
A t test of these values reports P = `r round( t.test(y1,y2)$p.value,3)`, while a Wilcoxon Rank test reports P = `r round( wilcox.test(y1, y2)$p.value, 3)`.

If we add an outlier to Y1, the results change dramatically for a t test but less so for the Wilcoxon Rank test:
