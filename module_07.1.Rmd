---
title: "Module 7.1: Counts and categories: The Binomial, Chi-squared and Fisher's Exact tests"
...
*This module shows methods for evaluating distributions of counts of the occurrences of categorical variables. First, it introduces the Binomial distribution, which can be used to characterize categorical variables with two possible values. It then introduces tests based on the chi-squared distribution, a method that generates approximations to the P value for categorical tests of one variable with two or more values, or two variable tests using contingency tables. Finally, Fisher's exact test, an exact method applicable to two-by-two contingency tables, is introduced.*
