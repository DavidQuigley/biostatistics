...
* How to summarize a single random real-valued variable
    + mean and median
    + percentiles
...
Here I have plotted a set of 10 values along the X axis. Their mean is `r round( mean(vals), 2)`, indicated by a solid vertical line.
...
legend( 25, 1.24, c("mean", "median"), lty=c(1,2), lwd=2, box.col="white", col=c("black","cornflowerblue") )
...
par(mar=c(2,3,1,1))
hist(vv2, las=1, xlim=c(0,20), main="Histogram, right skew")
box()
...
This plot shows the same data as in the previous distribution, but with all values less than 4.5 removed and more values above the median. The median has increased slightly, but note that the median is now almost identical to the 25th percentile, and the lower whisker is very short compared to the upper whisker. This indicates compresssion in the distribution.
